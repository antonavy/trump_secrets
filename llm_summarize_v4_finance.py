...
async def main():
    ...
    sample_posts = [
        ...
        "I will be imposing a 25% tariff on all goods from Mexico and Canada immediately!",
        "General Motors is doing a terrible job. Bring jobs back!",
        "The election was rigged! Total disgrace.",
        "Bitcoin is looking very strong. Keeping my eyes on Crypto!"
    ]

    analyzer = TrumpFeedAnalyzer()
    
    start = time.time()
    print(f"Processing {len(sample_posts)} posts in batches...")
    ...
    for res in results:
        # Only print what matters for money
        if res.get('is_financial_relevant'):
            impact = res.get('market_impact', {})
            print(f"🚨 ALERT [Score: {res.get('relevance_score')}/10]")
            print(f"Headline: {res.get('headline')}")
            print(f"Summary: {res.get('summary')}")
            print(f"Tickers: {impact.get('tickers')}")
            print(f"Sectors: {impact.get('sectors')}")
            print(f"Sentiment: {impact.get('sentiment')}")
            print("-" * 30)
        else:
            print(f"Context: {res.get('headline')} (Ignored: Low relevance)")
